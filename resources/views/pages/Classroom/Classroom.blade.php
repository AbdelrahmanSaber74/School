            ...
            <br><br>
             ...
             <form action="{{ route('Filter_Classes') }}" method="POST">
                ...
                <select class="selectpicker" data-style="btn-info" name="grade_id" required
                    ...
                    <option value="" selected hidden>{{ trans('My_Classes.Search_By_Grade') }}</option>
                    ...
                    <thead>
                        <tr>
                            ...
                            <th><input name="select_all" id="example-select-all" type="checkbox" onclick="CheckAll('box1', this)" /> </th>
                                  <th>#</th>
                                    @if (App()->getLocale() == 'ar')
                                    <th>{{ trans('My_Classes.Name_class_ar') }}</th>
                                    @else
                                    <th>{{ trans('My_Classes.Name_class_en') }}</th>
                                    @endif
                 ...
                 @endif
                        ...
                        @foreach ($list as $Classroom)
                            <tr>
                                ...
                                @if  (  app()->getLocale() == 'ar'  )
                                <td>{{ $Classroom->name_class_ar }}</td>
                                @else
                                <td>{{ $Classroom->name_class_en }}</td>
                                @endif

                                @if  (  app()->getLocale() == 'ar'  )
                                <td>{{ $Classroom->Grades->name_ar }}</td>
                                @else
                                <td>{{ $Classroom->Grades->name_en }}</td>
                                @endif
                                ...
                                <td>
                                    ...
                                    <button type="button" class="btn btn-info btn-sm" data-toggle="modal"
                                        ...
                                        title="{{ trans('My_Classes.edit_class') }}"><i class="fa fa-edit"></i></button>
                        ...
                        <!-- edit_form -->
                            ...
                            <div class="row">
                                <div class="col">
                                    <label for="name_class_ar"
                                        class="mr-sm-2">{{ trans('My_Classes.Name_class_ar') }}
                                        :</label>
                                    <input id="Name" type="text" name="name_class_ar"
                                        class="form-control"
                                        value="{{$Classroom->name_class_ar }}"
                                        required>
                            ...
                            <div class="form-group">
                                <label
                                    ...
                                    {{-- For Start Module Show Now grades --}}
                                    @if (App()->getLocale() == 'ar')
                                        <option  hidden value="{{$Classroom->Grades->id }}">
                                    ...
                                    {{-- For Selection Grades From Module --}}
                                @foreach ($Grades as $Grade)
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form action="{{ route('Classrooms.destroy', 'test') }}" method="post">
                                {{ method_field('Delete') }}
                                @csrf
                                {{ trans('My_Classes.Warning_class') }}
                                <input id="id" type="hidden" name="id" class="form-control"
                                    value="{{ $Classroom->id }}">
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary"
                                        data-dismiss="modal">{{ trans('My_Classes.Close') }}</button>
                                    <button type="submit"
                                        class="btn btn-danger">{{ trans('My_Classes.Delete') }}</button>
                                </div>
                                                </form>
                                            </div>
                                        </div>
        ...
        <div class="modal-content">
            ...
            <form action="{{ route('delete_all')}}" method="POST">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary"
                            data-dismiss="modal">{{ trans('My_Classes.Close') }}</button>
                    <button type="submit" class="btn btn-danger">{{ trans('My_Classes.Delete') }}</button>
                </div>
            </form>
        </div>
    </div>
